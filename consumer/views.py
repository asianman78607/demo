...
def category_data(request):
    ...
    context = {'seller_data':flx,'status':status,'l':l,'sk':'Category: '+sk,'typeu' : typeu,'lt': lt}
...
#MAIN PAGE
def main(request):
    global status,l,previous,sk,typeu,lt
...
def search_result(request):
    template = loader.get_template('item.html')
    context = {'seller_data':fl,'status':status,'l':l,'sk':sk,'typeu' : typeu,'lt': lt}
    return HttpResponse(template.render(context,request))
...
def item_detailed(request):
    ...
    context = {'seller_data':seller_data,'status':status,'comments':all_comments,'l':l,'sk':sk,'typeu' : typeu,'lt': lt,'p':p,'pl':pl}
    return HttpResponse(template.render(context,request))
...
def showcart(request):
    if request.user.is_authenticated:
        ...
        l=[]
        for x in data:
            l.append(x['cart_item'])

        data = products.objects.filter(id__in=l)
        context = {'seller_data':data,'status':status,'l':l,'sk':sk,'typeu' : typeu,'lt': lt}
        return HttpResponse(template.render(context,request))
    else :
        return HttpResponseRedirect('/c_login_form')
